...
export const FilterCard = ({data,index,category,setCategory}) =>{
    return(
      ...
      className={`group w-28 min-w-[128px] cursor-pointer rounded-md  py-6 ${
        category === data.category ? "bg-primaryColor" : "bg-primary"
      } hover:bg-primaryColor shadow-md flex flex-col items-center justify-center gap-4`}
    >
      <div
        className={`w-10 h-10 rounded-full shadow-md flex items-center justify-center group-hover:bg-primary ${
          category === data.category ? "bg-primary" : "bg-primaryColor"
        }`}
      >
        <IoFastFood
          className={`${
            category === data.category ? "text-primaryColor" : "text-primary"
