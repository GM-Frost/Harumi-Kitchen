...
const LoginInput = ({placeholder, icon, inputState,inputStateFunction, type, isSignUp}) => {
 const [isFocus, setIsFocus] = useState(false); 
    ...
    <motion.div 
        {...fadeInOut}
        className={`flex items-center justify-center gap-4 bg-lightOverlay backdrop-blur-md rounded-md w-full px-4 py-2 ${isFocus?"shadow-md shadow-primaryColor":"shadow-none"}`}>
        {icon}
        <input 
            type={type} 
