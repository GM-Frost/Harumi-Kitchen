      ...
      <nav className="flex items-center justify-center gap-8">
        <ul className="hidden md:flex items-center justify-center gap-16">
          ...
          >
           ...
           {cart?.length > 0 && (
             <div className='w-6 h-6 rounded-full bg-red-500 flex items-center justify-center absolute -top-4  -right-2 text-base font-semibold'>
         ...
         (
          ...
          <div className='relative cursor-pointer' onMouseEnter={()=>setIsMenu(true)}>
              <div className="ww-12 h-12 rounded-full shadow-md cursor-pointer overflow-hidden bg-green-200 flex items-center justify-center">
              ...
              {isMenu && (
                    ...
                    {(user?.user_id === (process.env.REACT_APP_ADMIN_1) || user?.user_id === (process.env.REACT_APP_ADMIN_2)) && (
                        <Link className='hover:text-primaryColor text-xl text-textColor' to={'/dashboard/home'}>Dashboard</Link>
                    )}
                    
                    <Link className='hover:text-primaryColor text-xl text-textColor' to={'/profile'}>My Profile</Link>
                    <Link className='hover:text-primaryColor text-xl text-textColor' to={'/user-orders'}>Orders</Link>
                    <hr/>
                    <motion.div 
                      {...buttonClick} 
                      ...
                      <p className='text-xl text-textColor group-hover::text-headingColor'>Log Out</p>
                    </motion.div>
                  </motion.div>
              )}
              </div>
            ...
            (
              ...
              <NavLink to={'/login'}>
                <motion.button
                  {...buttonClick}
                  className='px-4 py-2 shadow-md bg-lightOverlay border border-primaryColor cursor-pointer duration-100 transition-all ease-in-out'
                > 
                  Login
                </motion.button>
              </NavLink>
