  ...
  return (
    <div className='bg-lightOverlay hover:drop-shadow-lg backdrop-blur-md rounded-xl flex items-center justify-between relative px-4 py-2 w-full md:w-340 md:min-w-350 gap-3'>
         ...
         <p className="text-xl text-headingColor font-semibold">
                ...
                <p className='text-lg font-semibold text-primaryColor flex items-center justify-center gap-1'>
                    <HiCurrencyDollar className='text-primaryColor'/>{parseFloat(data.product_price).toFixed(2)}
                </p>

                <motion.div 
                    {...buttonClick}
                    onClick={sendToCart}
                    className="w-8 h-8 rounded-full bg-primaryColor flex items-center justify-center absolute -top-4 right-2 cursor-pointer">
